...
import { 
  ...
  ArrowRightLeft, ShieldAlert, X, Info, Zap, Clock, HelpCircle 
} from 'lucide-react';

function PassiveVoice() {
  return (
    <div className="text-slate-800 text-lg leading-relaxed space-y-12 pb-20">
      
      {/* NAGŁÓWEK */}
      <header className="border-b-2 border-amber-100 pb-6">
        <h1 className="text-4xl md:text-5xl font-black text-amber-600 mb-4">Passive Voice</h1>
        <p className="text-xl text-slate-500 font-medium">Strona Bierna. Przenosimy światło reflektorów z tego, KTO coś zrobił, na to, CO zostało zrobione. Opanowanie jej to klucz do poziomu B2/C1.</p>
      </header>
      ...
      <section className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl border border-slate-800">
        <h2 className="text-3xl font-black mb-6 text-amber-400 flex items-center gap-3">
          <BookOpen size={32} /> Złota Zasada (Zawsze 2 elementy)
        </h2>
      ...
      {/* PRZEGLĄD CZASÓW - GŁÓWNE MIĘSO */}
      <section>
        <h2 className="text-4xl font-black text-slate-800 mb-8 flex items-center gap-3 border-b-2 border-slate-100 pb-4">
          <Clock className="text-amber-500" size={40} /> Zmiany w zależności od czasu
        </h2>
        
        <p className="text-slate-600 mb-10 text-xl">Poniżej zobaczysz, jak strona bierna zachowuje się w różnych czasach. Zwróć uwagę, że główny czasownik (u nas: <strong>clean ➔ cleaned</strong>) dosłownie "stoi w miejscu", a całą magię wykonuje operator "to be".</p>
      ...
      {/* PUŁAPKI: BY VS WITH */}
      <section className="bg-rose-50 border border-rose-200 p-8 rounded-3xl mt-12">
      ...
      {/* DWA DOPEŁNIENIA */}
      <section className="bg-slate-100 p-8 rounded-3xl mt-12 border border-slate-200">
